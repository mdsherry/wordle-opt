...
impl LetterOutcome {
    fn idx(self) -> usize {
        match self {
            LetterOutcome::Miss => 0,
            LetterOutcome::Yellow => 1,
            LetterOutcome::Green => 2,
        }
    }
}
...
struct WordOutcome {
    letter_outcomes: [LetterOutcome; 5]
}

impl WordOutcome {
    fn new(guess: &str, target: &AugmentedAnswer<'_>) -> Self {
        let mut letter_outcomes = [LetterOutcome::Miss; 5];
        for (i, (guess_char, answer_char)) in guess.chars().zip(target.word.chars()).enumerate() {
            if guess_char == answer_char {
                letter_outcomes[i] = LetterOutcome::Green;
            } else if target.bitmask & (1 << (guess_char as u8 - b'a')) != 0 {
                letter_outcomes[i] = LetterOutcome::Yellow;
            }
        }
        Self { letter_outcomes }
    }

    fn idx(self) -> usize {
        self.letter_outcomes[0].idx() * 81 +
        self.letter_outcomes[1].idx() * 27 +
        self.letter_outcomes[2].idx() * 9 +
        self.letter_outcomes[3].idx() * 3 +
        self.letter_outcomes[4].idx()
    }
}
...
fn info<'a>(buckets: impl IntoIterator<Item=&'a u16>) -> Number {
    let mut total = 0.;
    let mut h = 0.;
    for &bucket in buckets {
        if bucket > 0 {
            // let p = (bucket as Number) / total;
            let p = bucket as Number;
            total += p;
            h += p * p.log2();
        }
    }
...
fn info_two(outcomes1: &Outcomes, outcomes2: &Outcomes, buckets: &mut FxHashMap<usize, u16>, idxs: &mut Vec<usize>) -> Number {
...
impl Outcomes {
    pub fn new(guess: &str, answers: &[AugmentedAnswer<'_>]) -> Self {
        let outcomes = answers.iter().map(|answer|
            WordOutcome::new(guess, answer).idx() as u8
        ).collect();
        Outcomes { outcomes }
    }
...
impl<'a> WordleOpt<'a> {
    ...
    pub fn best_two(&self) -> Option<(&str, &str, Number)> {
